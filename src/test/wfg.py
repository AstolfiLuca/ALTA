...
def test_wfg(algorithms, n_gen=200, n_obj=5, n_var=10, k=4, n=range(9), tight_layout=False):
    n_problems = len(n)
    ...
    for i, problem_num in enumerate(n):
        ...
        results = get_results(get_problem(problem_name, n_var=n_var, n_obj=n_obj, k=k), algorithms, n_gen)
